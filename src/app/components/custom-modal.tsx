...
const CustomModal = ({
  ...
  return (
    <Modal
      ...
      hideCloseButton
      classNames={{
        // wrapper: `${placement === 'top' && maxHeight && webApp?.platform !== 'android' ? `max-h-[60vh] overflow-y-auto no-scrollbar` : ''}`,
        base: full
    ...
    >
      <ModalContent>
        {(onClose) => (
          <div
            className={`max-w-[480px] mx-auto w-full ${full ? 'h-full' : 'overflow-y-auto no-scrollbar overscroll-y-none'}`}
          >
            {full ? (
              <>{background}</>
            ) : (
