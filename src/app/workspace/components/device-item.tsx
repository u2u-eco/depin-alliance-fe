          ...
          {Object.keys(listInfo)?.map((keyItem) => {
            return (
              <div className="space-y-2 xs:space-y-2.5 2xs:space-y-3" key={keyItem}>
                ...
                <div
                  className={`grid ${keyItem === 'RAM' ? 'grid-cols-3' : keyItem === 'STORAGE' || keyItem === 'CPU' ? 'grid-cols-1' : 'grid-cols-2'} gap-2`}
                >
                  {isLoading ? (
                      ...
                      {listInfo[keyItem].map((item: IDeviceTypeItem, index: number) => {
                        if (item.code) {
                          return (
                            <div
                              key={index}
                              className="flex items-center justify-center py-2 px-2 xs:px-3 2xs:px-4 bg-black/20 [clip-path:_polygon(12px_0%,100%_0,100%_calc(100%_-_12px),calc(100%_-_12px)_100%,0_100%,0_12px)] cursor-pointer text-xs xs:text-[13px] !leading-[16px] xs:!leading-[20px] 2xs:!leading-[24px] 2xs:text-sm whitespace-nowrap"
